...
class MicropubRenderTest extends WP_UnitTestCase {
	...
	function test_create_checkin_autogenerates_checkin_text_with_content() {
		...
		$post_content = \Micropub\Render::generate_post_content( 'something', $input );
	...
	function test_check_no_content_passed() {
		...
		$post_content = \Micropub\Render::generate_post_content( '', $input );
...
EOF
	...
	function test_merges_auto_generated_content() {
		...
		$post_content = \Micropub\Render::generate_post_content( 'foo bar', $input );
		$this->assertEquals(
			<<<'EOF'
